      ...
      {isHovered &&
        ...
        createPortal(
          <div className="flex flex-col absolute w-96 p-2 text-sm rounded-lg shadow-2xl z-20 bg-edBlue dark:bg-edDarker dark:border border-edDividerGray"
            ...
            <h2 className="text-center text-white dark:text-edWhite text-2xl border-b-2 pb-2 border-edWhite font-semibold">{title} Tips</h2>
            {helpTextList.map(helpText => {
              return <p className="px-2 py-4 border-b border-white dark:border-edDividerGray text-white dark:text-edWhite text-center whitespace-pre text-wrap">{helpText}</p>;
            })}
            <p className="py-4 text-white dark:text-edWhite text-center">Move your mouse outside of this window to close it.</p>
          </div>,
          document.body
        )
      }
